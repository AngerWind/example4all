...
public class IdempotentAspect {
    ...
    @Autowired
    private RedisLuaLimiterByList redisChecker;
    ...
    private String generateParm(Object[] args, Method method, Idempotent idempotent) {
        ...
        // value有值时取值作为key
            ...
            for (String keyTarget : keyTargets) {
                // 解析SpEL表达式
                Expression exp = parser.parseExpression(keyTarget);
                // 获取SpEL表达式的值
                Object value = exp.getValue(context, Object.class);
                // 组装参数
                append(result, keyTarget + "=" + value.toString());
            }
