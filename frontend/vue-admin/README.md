...
在 Vue3 中可以利用 useAttrs 方法获取组件的属性与事件(包含:原生 DOM 事件或者自定义事件),次函数功能类似于 Vue2 框架中\$attrs 属性与$listeners 方法。
...
进入到项目根目录 yarn 安装全部依赖.安装完依赖运行程序:yarn dev
...
```
yarn add --dev eslint-plugin-import eslint-plugin-vue eslint-plugin-node eslint-plugin-prettier eslint-config-prettier eslint-plugin-node @babel/eslint-parser
```
...
```
"scripts": {
    "lint": "eslint src", // 校验语法
    "fix": "eslint src --fix", // 校验并自动纠正
}
```
...
**通过 yarn format 格式化代码，如果出现不规范格式,通过 yarn fix 修改**

#### 三、配置 stylelint

[stylelint](https://stylelint.io/)为 css 的 lint 工具。可格式化 css 代码，检查 css 语法错误与不合理的写法，指定 css 书写顺序等。

我们的项目中使用 sass 作为预处理器，安装以下依赖：

```
yarn add --dev sass sass-loader stylelint postcss postcss-scss postcss-html stylelint-config-prettier stylelint-config-recess-order stylelint-config-recommended-scss stylelint-config-standard stylelint-config-standard-vue stylelint-scss stylelint-order stylelint-config-standard-scss
```
...
```
 "scripts": {
    ...
    "lint": "eslint src",
    "fix": "eslint src --fix",
    "format": "prettier --write \"./**/*.{html,vue,ts,js,json,md}\"", // 格式化代码
    "lint:eslint": "eslint src/**/*.{ts,vue} --cache --fix", // 校验ts语法
    "lint:style": "stylelint src/**/*.{css,scss,vue} --cache --fix" // 校验sass,css语法
  },
```
...
初始化 hosky, 会在根目录下生成个一个.husky 目录, 用于存放需要执行的脚本
...
执行`yarn husky add .husky/pre-commit "yarn format"`来创建一个 pre-commit 文件, 该文件会在 git commit 执行被执行
...
配置 husky, 执行如下命令, 会在.husky 目录下创建以 commit-msg 文件, 内容如下

```shell
yarn husky add .husky/commit-msg "yarn commitlint"
```

之后我们执行 commit 的时候就会执行该脚本, 来校验我们的 commit massage
只有 git commit -m 'fix: xxx' 符合类型的才可以，**需要注意的是类型的后面需要用英文的 :，并且冒号后面是需要空一格的，这个是不能省略的**
...
**当我们使用 npm 或者 pnpm 来安装包的时候，就会报错了。原理就是在 install 的时候会触发 preinstall（npm 提供的生命周期钩子）这个文件里面的代码。**
...
```
//@ts-ignore 忽略当前文件ts类型的检测否则有红色提示(打包会失败), 因为zh-cn.mjs没有类型声明文件
import zhCn from 'element-plus/dist/locale/zh-cn.mjs'
// 使用插件, 配置elementplus使用中文
app.use(ElementPlus, {
    locale: zhCn
})
```
...
VITE_APP_TITLE = '硅谷甄选运营平台' #开发环境下标题变量
VITE_APP_BASE_API = '/dev-api'	#开发环境下的基础路径
VITE_SERVE="http://xxxx.com"   #开发环境下的服务器的地址
```
...
通过 import.meta.env 获取环境变量, 默认为 development 环境
...
```
yarn add --dev vite-plugin-svg-icons
```
...
**下载图标库并使用**
图标库的下载可以从https://www.iconfont.cn/collections/index网站上下载
选中 svg 图标后, 点击下载, 然后赋值到 src/assets/icons 目录下
然后在 vue 文件中可以使用如下方式进行使用

```html
<!-- 放在svg的容器 -->
<svg style="width: 30px; height: 100px">
  <!--xlink:href指定使用哪个图标, 属性值是#icon-图标名字-->
  <use xlink:href="#icon-eps" fill="yellow"></use>
</svg>
```

#### 3.4.1 svg 封装为全局组件 (该项目中没有完成这个步骤)
...
在 src 文件夹目录下创建一个 index.js 文件：用于注册 components 文件夹内部全部全局组件！！！
...
在 src/styles 目录下创建一个 index.scss 文件，用于保存所有的全局样式
在 main.ts 入口文件引入
...
在 index.scss 中引入 variable.scss
...
```
yarn add --dev vite-plugin-mock@2.9.6 mockjs
```
