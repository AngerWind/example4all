package com.example.example4springboot.controller;
...
public class WidgetRestController {
    ...
    public ResponseEntity<?> getWidget(@PathVariable("id") Long id) {
    ...
    public ResponseEntity<Widget> updateWidget(@RequestBody Widget widget, @PathVariable("id") Long id, @RequestHeader(HttpHeaders.IF_MATCH) String ifMatch) {
    ...
    public ResponseEntity<Widget> updateWidgetProper(@RequestBody Widget widget, @PathVariable("id") Long id, @RequestHeader("If-Match") Integer ifMatch) {
        Optional<Widget> existingWidget = widgetService.findById(id);
        if (existingWidget.isPresent()) {
