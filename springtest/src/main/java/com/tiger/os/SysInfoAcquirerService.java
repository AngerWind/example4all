 ...
 * @date 2017-4-11
 */
@Slf4j public class SysInfoAcquirerService {
    ...
    /**
     * 初始化bean的时候就立即获取JVM进程的PID及执行任务 * * @return
     */
    ...
    /**
     * 获取JVM 的CPU占用率（%） * * @return
     */
    public String getCPURate() {
        String cpuRate = "";
        if ("windows".equals(isWindowsOrLinux)) {
            // 判断操作系统类型是否为：windows cpuRate = getCPURateForWindows();
        } else {
    ...
    /** * 获取磁盘读写速率（MB/s） * * @return */
    // public String getDiskAccess() {
    // TODO:待完成
    // String diskAccess = "";
    // if (isWindowsOrLinux.equals("windows")) {
    // 判断操作系统类型是否为：windows
    //
    // diskAccess = getDiskAccessForWindows();
    // 查询windows系统的磁盘读写速率
    // } else {
    // diskAccess = getDiskAccessForLinux();
    // 查询linux系统的磁盘读写速率
    // }
    // return diskAccess;
    // }
    ...
    public String getNetworkThroughputForWindows() {
        ...
        try {
            ...
            rxPercent = formatNumber(
                (Long.parseLong(result2[0]) - Long.parseLong(result1[0])) / (float)(1024 * 1024 * (SLEEP_TIME / 1000)));
            // 上行速率(MB/s)
            txPercent = formatNumber(
                (Long.parseLong(result2[1]) - Long.parseLong(result1[1])) / (float)(1024 * 1024 * (SLEEP_TIME / 1000)));
            // 下行速率(MB/s)
            input.close();
            pro1.destroy();
            pro2.destroy();
    ...
    /**
     * 格式化浮点数(float 和 double)，保留两位小数 * * @param obj * @return
     */
    ...
    /**
     * 测试方法 ：监测java执行相关命令后是否能获取到结果集(注：此方法执行后会中断程序的执行，测试完后请注释掉) * * @param br
     */
    ...
    class SysInfoAcquirerTimerTask extends TimerTask {
        ...
        @Override public void run() {
            try {
                ...
                //                sysPerformInfoService.insertPerformInfo(sysPerformInfo);
                // 将采集到的数据插入数据库
    ...
    @Data public static class SysPerformInfo {
        String Id;
        Float downSpeed;
        Float upSpeed;
        Integer threadCount;
    ...
    public static void main(String[] args) {
        ...
        sysInfoAcquirerService.getNetworkThroughput();
        sysInfoAcquirerService.getMemoryRateForLinux();
