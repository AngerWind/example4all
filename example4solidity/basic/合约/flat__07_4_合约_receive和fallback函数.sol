...
/** 
 *  SourceUnit: c:\Users\Administrator\Desktop\example4all\example4solidity\basic\合约\_07_4_合约_receive和fallback函数.sol
*/
...
/**
    receive接收以太:
        ...
        可以是virtual, 可以被重写, 也可以有修改器
        在对合约没有任何附加数据调用（通常是对合约转账）是会执行 receive 函数．　
            例如　通过 .send() or .transfer() or address{value: 100}.call(""), 即便发送0eth
        如果 receive 函数不存在，就会调用payable的fallback回退函数
        如果两个函数都没有，这个合约就没法通过常规的转账交易接收以太（会抛出异常）

        !!! receive中不要执行太多逻辑, 因为send和transfer固定gas 2300, 太多逻辑可能会导致out of gas

    fallback回退函数:
        合约可以最多有一个回退函数。函数声明为： 
            fallback () external [payable]
            fallback (bytes calldata input) external [payable] returns (bytes memory output)
        可以是virtual的, 可以被重写, 可以有修改器
        如果其他合约对当前合约的调用中, 没有函数与给定的函数标识符匹配, fallback会被调用
        或者当其他合约向当前合约发送以太, 而当前合约没有receive函数时, 会调用payable的fallback函数 (****eth会被接收****** )
