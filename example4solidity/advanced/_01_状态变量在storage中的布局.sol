 ...
 * 规整如下:
 * 1. 每个槽位占32字节
 * 2. 多个变量按照c3线性化的顺序存储在槽位中
 * 3. 如果相邻的多个变量可以存储在同一个槽位中，那么他们就会被存储在同一个槽位中, 
 *      如果存储插槽storage slot 中的剩余空间不足以储存一个值类型，那么它会被存入下一个 存储插槽storage slot 。
 * 
 * 4. 值类型值仅占存储他们所需的字节, 
 *          如uint8占一个字节, address占20字节, bool占1字节, 
 *          bytes4占4字节, bytes32占32字节
 *          enum按照uint8处理,
 * 5. 碰到结构体和数组(string和bytes也算), 总是分配一个新的槽位
 * 6. 结构体中会像成员变量单独声明一样进行槽位分配
 * 7. 数组总是单独槽位, 无论是静态数组还是动态数组
 *          静态数组类似uint8[3], 数据直接编码在槽位中其中的元素
 *          如果一个元素占用少于16字节, 那么多个元素会共享slot
 * 
 * 8. 由于 映射mapping 和动态数组不可预知大小，不能在状态变量之间存储他们。
 *      相反，他们自身根据以上规则仅占用 32 个字节，然后他们包含的元素的存储的起始槽位，则是通过 Keccak-256 哈希计算来确定。
 *
 *      假设 映射mapping 或动态数组根据上述存储规则最终可确定某个位置 p 。 
 *      对于动态数组，此插槽中会存储数组中元素的数量
 *      对于 映射mapping ，该插槽全部是0
 * 
 ...
 * 9. 对于string和bytes, 如果长度小于32字节, 会直接编码在槽位中, 数据存储在高位, 最低位存储length*2
 *      如果长度高于32直接, 那么低位存储length*2+1, 数据从槽位keccak256(p) 开始, 所以可以判断低位奇偶来判断他是否大于32字节
 * 
 * 槽位中的值可以使用ethers的provider.getStorageAt()来获取Compilation Details > storagelayout中查看
 * 合约的槽位的布局可以在remix的
 */
